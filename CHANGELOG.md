...
- **Release tags are now bare version numbers** (`2.0.0`, not `v2.0.0`), matching `manifest.version` and nf-core convention; the `v` belongs only in the displayed release name. The existing `v1.0`–`v1.2.0` tags are untouched, so `-r v1.1.7` keeps working, but new releases are pinned as `-r 2.0.0`.
